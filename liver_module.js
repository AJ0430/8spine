...
function loadGeo(force) {
  ...
  geoFetching = fetch(CFG.geolier, { headers: UA }).then(function (res) {
    ...
    var m5 = src.match(/function\s+md5\s*\([^)]*\)\s*\{/);
    if (m5) {
      var open = src.indexOf("{", m5.index);
      var depth = 0, end = -1;
      ...
      if (end > open) {
        try {
          var f = new Function("return (" + src.slice(m5.index, end + 1) + ")");
          var fn = f();
          if (typeof fn === "function" && fn("abc") === "900150983cd24fb0d6963f7d28e17f72") g.md5 = fn;
        } catch (e) { g.md5 = null; }
      }
    }
    geoCache = g;
